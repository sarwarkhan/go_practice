...
func handleClient(conn net.Conn) {
	...
	SMS_HOST_PASS := "*****"
	SMS_SENDER := "****"
	SMS_API := fmt.Sprint("api_url=", SMS_HOST_USER, "&password=", SMS_HOST_PASS, "&sender=", SMS_SENDER)
	...
	for {
		...
		/* Open DB connection */
		db, dbError := sql.Open("mysql", "user_name:password@/db_name")
		if dbError != nil {
			...
			if startBits == "7878" { // normal data
				...
				if incomingDataProtocol == "13" && loginState == true && dataType == "A" {
					...
					if sensorDataConversionError != nil {
						fmt.Println("13-> Sensor data conversion error for data-protocol 13: " + terminalId)
						continue
					}
					...
					if engineErr != nil {
						fmt.Println("13-> Engine status error for data-protocol 13: " + terminalId)
						continue
					}
					engineStatus = engine
					if engineStatus == 0 {
					...
					if locationDataFlag == true {
						...
						//fmt.Println(fmt.Sprint("13->Location data time difference : ", lastLocationDataTimeDiff))
						if lastLocationDataTimeDiff >= 3.00 {
							...
							if stmtError != nil {
								fmt.Println(fmt.Sprint("13-> Prepared error: ", stmtError.Error()))
								continue
							}
							...
							if execError != nil {
								fmt.Println(fmt.Sprint("13-> Query execution error: ", execError.Error()))
								continue
							} else {
								_ = dbResult
							}
							/*Print*/
							fmt.Println(fmt.Sprint("** heart-bit data : ", terminalId, " at ", dateTimeFormated))
						}
					}
				}
				/* handle location data */
				if incomingDataProtocol == "12" && loginState == true {
					if len(incomingDataPacket) < 60 {
						fmt.Println("12-> Invalid data-packet length for location data")
						continue
					}
					//update previous location data update time
					locationDataLastTime = time.Now()
					...
					if dateTimeFormated == "" {
						fmt.Println("12->Datetime conversion error from hex string")
						continue
					}
					...
					if latConverErr != nil {
						fmt.Println("12->Latitude conversion error from hex string")
						continue
					}
					...
					if lonConverErr != nil {
						fmt.Println("12->Longitude conversion error from hex string")
						continue
					}
					...
					if speedConverErr != nil {
						fmt.Println("12->Speed conversion error from hex string")
						continue
					}
					...
					if courceErr != nil {
						fmt.Println("12->Course and status conversion error from hex string")
						continue
					}
					if byteBinary[4:5] == "0" {
						e_w_indicator = "E"
					...
					preparedStmt, stmtError := db.Prepare(insertSQL)
					if stmtError != nil {
						fmt.Println(fmt.Sprint("12-> Prepared statement error: ", stmtError.Error()))
						continue
					}
					//execute prepared statement
					dbResult, execError := preparedStmt.Exec(terminalId, dateFormated, dateTimeFormated,
						...
						defenceStatus, voltageLevelStatus, gsmSignalStrength, alarmLanguage)
					if execError != nil {
						fmt.Println(fmt.Sprint("12-> Query execution error: ", execError.Error()))
						continue
					} else {
						_ = dbResult
					}
								...
								if elegibleForOverSpeedSMS {
									...
									if addressError == nil {
										...
										if isSMSLogUpdated {
											...
											//send sms
											http.Get(smsApiUrl)
											fmt.Println("12-> SMS sent for over speed")
										}
									}
								}
							}
								...
								if weekDayFlag {
									...
									if geofenceAlartSMSFlag {
										...
										if isSMSLogUpdated {
											...
											//send sms
											http.Get(smsApiUrl)
											fmt.Println("12-> SMS sent for geo-fence: " + geofenceSMSStatus)
										}
									}
								}
							}
				...
				if incomingDataProtocol == "16" && loginState == true {
					if len(incomingDataPacket) < 62 {
						fmt.Println("16-> Invalid data-packet length for alarm data")
						continue
					}
					//update previous location data update time
					locationDataLastTime = time.Now()
					...
					if dateTimeFormated == "" {
						fmt.Println("16->Datetime conversion error from hex string")
						continue
					}
					dateFormatedArrayLocation := strings.Split(dateTimeFormated, " ")
					dateFormated = dateFormatedArrayLocation[0]
					...
					latitudeDecimal, latConverErr := hex2Int(hexLatitude)
					if latConverErr != nil {
						fmt.Println("16->Latitude conversion error from hex string")
						continue
					}
					latitudeInDecimalMinutes = (float64(latitudeDecimal) / 30000) / 60
					/*print*/
					...
					longitudeDecimal, lonConverErr := hex2Int(hexLongitude)
					if lonConverErr != nil {
						fmt.Println("16->Longitude conversion error from hex string")
						continue
					}
					longitudeInDecimalMinutes = (float64(longitudeDecimal) / 30000) / 60
					/*Print*/
					...
					speedFromHex, speedConverErr := hex2Int(hexSpeed)
					if speedConverErr != nil {
						fmt.Println("16->Speed conversion error from hex string")
						continue
					}
					speedInDecimal = float64(speedFromHex)
					/*Print*/
					...
					byteBinary, courceErr := hex2Bin(hexCourseStatus)
					if courceErr != nil {
						fmt.Println("16->Course and status conversion error from hex string")
						continue
					}
					if byteBinary[4:5] == "0" {
						e_w_indicator = "E"
					...
					sensorDataBinary, sensorDataConversionError := hex2Bin(terminalStatus)
					if sensorDataConversionError != nil {
						fmt.Println("16-> Sensor data conversion error for terminal: " + terminalId)
						continue
					}
					...
					engine, engineErr := strconv.Atoi(sensorDataBinary[6:7])
					if engineErr != nil {
						fmt.Println("16-> Engine status error for terminal: " + terminalId)
						continue
					}
					engineStatus = engine
					defenceStatus = sensorDataBinary[7:8]
					...
					preparedStmt, stmtError := db.Prepare(insertSQL)
					if stmtError != nil {
						fmt.Println(fmt.Sprint("16-> Prepared statement error: ", stmtError.Error()))
						continue
					}
					//execute prepared statement
					dbResult, execError := preparedStmt.Exec(terminalId, dateFormated, dateTimeFormated,
						...
						defenceStatus, voltageLevelStatus, gsmSignalStrength, alarmLanguage)
					if execError != nil {
						fmt.Println(fmt.Sprint("16-> Query execution error: ", execError.Error()))
						continue
					} else {
						_ = dbResult
					}
